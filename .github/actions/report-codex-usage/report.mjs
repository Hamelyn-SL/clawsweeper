/**
 * Codex token-usage reporter. Reads the rollout JSONL files Codex writes
 * under this run's CODEX_HOME, takes each session's final token_count event
 * (cumulative totals + account rate-limit snapshot), appends a table to the
 * job summary, and POSTs the events to the fleet llm-usage sink.
 *
...
function extractSession(path) {
  ...
  const event = {
    ...
    ...(lastUsage.reasoning_output_tokens !== undefined
      ? { reasoning: lastUsage.reasoning_output_tokens }
      : {}),
  ...
  if (typeof primary?.used_percent === "number") {
    event.rateLimit5h = primary.used_percent;
  }
  if (typeof secondary?.used_percent === "number") {
    event.rateLimitWeek = secondary.used_percent;
  }
  if (typeof lastRateLimits?.plan_type === "string") {
    event.plan = lastRateLimits.plan_type;
  }
  return event;
}
...
function writeSummary(events) {
  ...
  if (events.length === 0) {
    ...
    for (const e of events) {
      const burn = Math.max(0, e.input - e.cached) + e.output;
      burnTotal += burn;
      md += `| ${e.sessionId.slice(0, 8)}… | ${e.model ?? "?"} | ${fmt(e.input)} | ${fmt(e.cached)} | ${fmt(e.output)} | **${fmt(burn)}** | ${e.rateLimit5h ?? "?"}% | ${e.rateLimitWeek ?? "?"}% |\n`;
    }
...
async function postToSink(events) {
  ...
  if (!res.ok) {
    console.log(`::warning::llm-usage sink responded ${res.status}`);
  } else {
    console.log(`reported ${events.length} session(s) to llm-usage sink`);
  }
}
